...
def play_sound(event_type):
    ...
    def beep():
        if event_type == "clutch":
            ...
            # Changed to a higher frequency descending beep so laptop speakers can actually play it
...
class SetupWizard:
    def __init__(self, detector, cap, cam_width, cam_height):
        ...
        # Increased window size to prevent cutting off text
    ...
    def page_intro(self):
        self.clear()
        tk.Label(self.root, text="Welcome to the Virtual Mouse Calibration!", font=("Helvetica", 18, "bold")).pack(pady=40)
        tk.Label(self.root, text="A quirky, scale-invariant, dual-hand clutch masterpiece.", font=("Helvetica", 13)).pack(pady=10)
        tk.Label(self.root, text="Let's get your hands calibrated for absolute precision.", font=("Helvetica", 13)).pack(pady=10)
        tk.Button(self.root, text="Next >", command=self.page_instructions, font=("Helvetica", 13), bg="lightblue", width=15).pack(pady=50)

    def page_instructions(self):
        self.clear()
        tk.Label(self.root, text="Why do we need to configure?", font=("Helvetica", 18, "bold")).pack(pady=30)
        ...
        tk.Label(self.root, text=instructions, font=("Helvetica", 12), justify="left").pack(pady=20)
    ...
    def page_input(self):
        self.clear()
        tk.Label(self.root, text="Confirm Your Values", font=("Helvetica", 18, "bold")).pack(pady=30)
        ...
        tk.Label(frame, text="Left Click (Index) Ratio:", font=("Helvetica", 13)).grid(row=0, column=0, pady=15, padx=15, sticky="e")
        left_entry = tk.Entry(frame, textvariable=self.left_var, font=("Helvetica", 13), width=10)
        left_entry.grid(row=0, column=1, pady=15)
        
        tk.Label(frame, text="Right Click (Middle) Ratio:", font=("Helvetica", 13)).grid(row=1, column=0, pady=15, padx=15, sticky="e")
        self.right_entry = tk.Entry(frame, textvariable=self.right_var, font=("Helvetica", 13), width=10)
        self.right_entry.grid(row=1, column=1, pady=15)
        ...
        # Cleaned up checkbox text and bumped font size to scale the box up naturally
        ...
        tk.Button(self.root, text="Next >", command=self.page_ready, font=("Helvetica", 13), bg="lightblue", width=15).pack(pady=30)
    ...
    def page_ready(self):
        ...
        tk.Label(self.root, text="ARE YOU READY FOR THE PROTOTYPE?", font=("Helvetica", 16, "bold")).pack(pady=15)
        ...
        tk.Label(self.root, text=caution, font=("Helvetica", 10, "bold"), fg="#d9534f", justify="center").pack(pady=15)
        
        tk.Button(self.root, text="Continue", command=self.root.destroy, font=("Helvetica", 14, "bold"), bg="lightgreen", width=25).pack(pady=15)
        
        # Hidden Email Link
        def open_mail(e):
            webbrowser.open("mailto:rahanrazah@gmail.com")
