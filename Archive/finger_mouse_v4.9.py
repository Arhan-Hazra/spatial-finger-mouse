...
# --- SILENCE MEDIAPIPE TELEMETRY LOGS ---
...
# --- FIXED: HARDWARE-LEVEL WINDOWS MOUSE INJECTION CONFIG ---
# Direct API movement injection allows seamless brush tracking when a click state is active.
# We use mouse_event instead of SetCursorPos to synthesize a raw hardware interrupt.
def win32_move_mouse(x, y, s_width, s_height):
    # Windows hardware interrupts require absolute normalized coordinates (0 to 65535)
    # We map your screen resolution to this raw hardware scale
    abs_x = int((x * 65535) / s_width)
    abs_y = int((y * 65535) / s_height)
    
    # 0x8001 combines Absolute Positioning (0x8000) + Hardware Move Interrupt (0x0001)
...
# Configurable Eye Engine Parameters (Initialized with defaults)
...
class SetupWizard:
    ...
    def page_input(self):
        global gesture_timeout, max_blink_duration
        self.clear()
        tk.Label(self.root, text="Confirm Calibration Values", font=("Helvetica", 18, "bold")).pack(pady=(0, 20))
        
        self.left_var = tk.StringVar(value="0.20")
        self.right_var = tk.StringVar(value="0.20")
        self.timeout_var = tk.StringVar(value=str(gesture_timeout))
        self.blink_dur_var = tk.StringVar(value=str(max_blink_duration))
        ...
        tk.Label(frame, text="Max Blink Duration (s):", font=("Helvetica", 13)).grid(row=3, column=0, pady=8, padx=10, sticky="e")
        tk.Entry(frame, textvariable=self.blink_dur_var, font=("Helvetica", 13), width=10).grid(row=3, column=1, pady=8)
        
        tk.Button(self.root, text="Next >", command=self.page_ready, font=("Helvetica", 13), bg="lightblue", width=15).pack(pady=25)
    ...
    def page_ready(self):
        ...
        try:
            ...
            gesture_timeout = float(self.timeout_var.get())
            max_blink_duration = float(self.blink_dur_var.get())
        except ValueError:
            messagebox.showerror("Error", "Please make sure all entries are valid numbers!")
            self.page_input()
            return
...
if __name__ == "__main__":
    ...
    # FIX: Dropped deadzone down from 4 to 1 for perfectly organic brush stroke vectors
    deadzone = 1 
    ...
    try:
        while True:
            ...
            # --- PROCESS EYE ENGINE (ASYNC THREAD TRIGGERS) ---
            if eye_engine_active:
            ...
            if active_landmarks:
                ...
                # Right Click Engine
                if dist_right_ratio < right_ratio_in:
                    right_buffer = 0
                    if not right_held:
            ...
            if show_debug:
                ...
                if eye_engine_active:
                    if blink_detected: cv2.putText(img, "BLOCKED / CLOSED", (20, 85), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
