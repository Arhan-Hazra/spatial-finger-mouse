...
_dev_signature_alpha = "MOUSE POINTER ~ MADE BY none"
...
def run_cv2_configurator(mode, detector, cap, cam_width, cam_height):
    ...
    # OpenCV Mouse Callback to build an on-screen button
    ...
    while not next_clicked:
        ...
        if detection_result.hand_landmarks:
            ...
            # 1-Second Rolling Queue for Max/Min
            curr_time = time.time()
...
class SetupWizard:
    ...
    def page_intro(self):
        self.clear()
        tk.Label(self.root, text="Welcome to the Virtual Mouse Calibration!", font=("Helvetica", 16, "bold")).pack(pady=30)
        tk.Label(self.root, text="A quirky, scale-invariant, dual-hand clutch masterpiece.", font=("Helvetica", 12)).pack(pady=10)
        tk.Label(self.root, text="Let's get your hands calibrated for absolute precision.", font=("Helvetica", 12)).pack(pady=10)
        tk.Button(self.root, text="Next >", command=self.page_instructions, font=("Helvetica", 12), bg="lightblue", width=15).pack(pady=40)

    def page_instructions(self):
        self.clear()
        tk.Label(self.root, text="Why do we need to configure?", font=("Helvetica", 16, "bold")).pack(pady=20)
        ...
        tk.Label(self.root, text=instructions, font=("Helvetica", 11), justify="left").pack(pady=10)
        tk.Button(self.root, text="Next >", command=self.page_config_index, font=("Helvetica", 12), bg="lightblue", width=15).pack(pady=20)

    def page_config_index(self):
        self.root.withdraw() # Hide UI
        run_cv2_configurator("index", self.detector, self.cap, self.cam_width, self.cam_height)
        self.root.deiconify() # Bring UI back
    ...
    def page_input(self):
        self.clear()
        tk.Label(self.root, text="Confirm Your Values", font=("Helvetica", 16, "bold")).pack(pady=20)
        ...
        tk.Label(frame, text="Left Click (Index) Ratio:", font=("Helvetica", 12)).grid(row=0, column=0, pady=10, padx=10, sticky="e")
        left_entry = tk.Entry(frame, textvariable=self.left_var, font=("Helvetica", 12), width=10)
        left_entry.grid(row=0, column=1, pady=10)
        
        tk.Label(frame, text="Right Click (Middle) Ratio:", font=("Helvetica", 12)).grid(row=1, column=0, pady=10, padx=10, sticky="e")
        self.right_entry = tk.Entry(frame, textvariable=self.right_var, font=("Helvetica", 12), width=10)
        ...
        tk.Checkbutton(self.root, text="[ ] - ARE BOTH SAME VALUE?", variable=self.sync_var, command=sync_check, font=("Helvetica", 11)).pack(pady=10)
        tk.Button(self.root, text="Next >", command=self.page_ready, font=("Helvetica", 12), bg="lightblue", width=15).pack(pady=20)
    ...
    def page_ready(self):
        ...
        tk.Label(self.root, text="ARE YOU READY FOR THE PROTOTYPE?", font=("Helvetica", 14, "bold"), fg="red").pack(pady=20)
        
        notes = (
            "The camera window will automatically DISABLE to save resources.\n\n"
            "SHORTCUTS:\n"
            "• Ctrl + Alt + D : Toggle Camera Debug Window\n"
            "• Ctrl + Alt + H : Switch Hand Modes (Left/Right)"
        )
        tk.Label(self.root, text=notes, font=("Helvetica", 11), justify="center").pack(pady=20)
        tk.Button(self.root, text="Continue", command=self.root.destroy, font=("Helvetica", 12, "bold"), bg="lightgreen", width=20).pack(pady=30)

# [ HEADING: CORE LOGIC WATERMARK ]
...
if __name__ == "__main__":
    ...
    # Init camera globally so it passes between UI and engine smoothly
    ...
    # Camera auto-disables upon entering mouse mode!
    show_debug = False 
    ...
    # 3. RUN MOUSE LOOP (With bulletproof Graceful Shutdown)
    try:
        while True:
            success, img = cap.read()
            if not success: break
        ...
        try:
            ...
            pass
        # This completely obliterates the terminal process, ensuring NO red tracebacks ever appear
        os._exit(0)