    ...
    try:
        while True:
            ...
            if clutch_engaged:
                ...
                # FIX: When clutch is turned off, we don't wipe out prev_x/prev_y!
                # This ensures tracking resumes perfectly without jumpy snaps.
                was_clutched = False
            ...
            if active_landmarks:
                ...
                # --- TRACKING / MOVEMENTS ---
                # FIX: We only block cursor *coordinate adjustments* while clutching.
                # Click management stays exposed so that your dragging/holding states carry through!
                if not clutch_engaged:
                    ...
                    # While clutching, update history silently to match hand position without moving pointer.
                    # This prevents the cursor from jumping wildly when the clutch is released.
                    prev_x, prev_y = mid_knuckle[0], mid_knuckle[1]

                # --- CLICK CONTROLLERS (Exposed outside clutch freeze) ---
                ...
                # If tracking hand leaves field entirely, reset anchor coordinates
                # But notice: we NO LONGER force mouseUp here. Clicks persist safely.
                prev_x = None
                if hand_was_present:
                    play_sound("lost")
                    hand_was_present = False
