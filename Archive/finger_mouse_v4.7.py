...
import webbrowser
import ctypes  # For kernel-level mouse injection (Fixes Painting/Drawing)
...
# --- WINDOWS MOUSE INJECTION CONFIG ---
...
if __name__ == "__main__":
    ...
    # --- FIXED EYE ENGINE REGISTRATION ---
    # Shifted running mode to static IMAGE to process inline synchronously. 
    # This prevents thread-skipping that was rendering blinks broken/unresponsive.
    ...
    try:
        while True:
            ...
            # --- PROCESS EYE ENGINE (SYNCHRONIZED INLINE LOOP) ---
            if eye_engine_active:
            ...
            if clutch_engaged:
                ...
                # Re-engage cleanly upon clutch exit without jumping coordinate states
                if was_clutched:
                    was_clutched = False
                    prev_x, prev_y = None, None  # Re-anchor tracking reference smoothly
                ...
                if not clutch_engaged:
                    ...
                    if prev_x is None:
                        ...
                        if abs(move_x) > deadzone or abs(move_y) > deadzone:
                            ...
                            # --- FIX: PAINTING AND DRAWING SUPPORT ---
                            # If a click button is active, bypass standard pyautogui.moveTo and push coordinates 
                            # directly into user32.dll core via ctypes. This protects line rendering from breaking.
                    ...
                    # FIX: While clutch is active, completely decouple coordinate updating from historical layers.
                    # This removes micro-stutters when returning tracking hand.
                    pass
