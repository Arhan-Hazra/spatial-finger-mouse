...
def play_sound(event_type):
    ...
    def beep():
        if event_type == "clutch":
            winsound.Beep(900, 100)  # High pitch
            winsound.Beep(1200, 100) # Higher pitch (Success)
        elif event_type == "lost":
            winsound.Beep(300, 200)  # Low pitch (Error boop)
...
try:
    left_ratio_in = float(input("Enter Left Click Ratio (Index-Thumb) [e.g., 0.20]: "))
    right_ratio_in = float(input("Enter Right Click Ratio (Middle-Thumb) [e.g., 0.20]: "))
except ValueError:
    print("Invalid input. Defaulting to 0.20")
    left_ratio_in = 0.20
    right_ratio_in = 0.20
...
active_hand = "Right" 
was_clutched = False
hand_was_present = False # Track if hand was visible last frame
...
# Wrap the main loop in a Try block to elegantly catch Ctrl+C
try:
    while True:
        ...
        if clutch_engaged:
            if not was_clutched:
                play_sound("clutch") # Play sound only the exact moment it engages
            was_clutched = True
        ...
        # --- ACTIVE HAND LOGIC ---
        if active_landmarks:
            hand_was_present = True # Record that the hand is safely in view
            def get_px(landmark): return int(landmark.x * cam_width), int(landmark.y * cam_height)
            ...
            prev_x = None
            # Trigger the "lost" sound if the hand disappears
            if hand_was_present:
                play_sound("lost")
...
# This finally block guarantees the camera and mouse buttons reset safely
# even if you press Ctrl+C or force quit the terminal!
finally:
